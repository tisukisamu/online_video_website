<template>
  <div class="min-h-screen bg-black text-white pb-20">
    <div class="fixed top-0 left-0 right-0 z-50 flex items-center px-4 h-[60px] bg-black border-b border-white/10">
      ...
      <span class="text-white text-base font-medium ml-4">粉丝 ({{ total }})</span>
    </div>
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const res = await followApi.getUserFollowers(userId.value, page.value, 20)
    if (res.code === 200) {
